...
function fewestCoinChange(cents) {
  ...
  coincount = { 'quarter' : 0, 'dime' : 0, 'nickel': 0, 'penny': 0}

  while (cents >= coin['quarter']){
    cents = cents - coin['quarter'];
    coincount['quarter'] += 1;
  }
  while (cents >= coin['dime']){
    cents = cents - coin['dime'];
    coincount['dime'] += 1;
  }
  while (cents >= coin['nickel']){
    cents = cents - coin['nickel'];
    coincount['nickel'] += 1;
  }
  while (cents >= coin['penny']){
    cents = cents - coin['penny'];
    coincount['penny'] += 1;
  }
console.log(coincount)
  // if value in coincount['key'] == 0; remove key + value;  
  return coincount
}
